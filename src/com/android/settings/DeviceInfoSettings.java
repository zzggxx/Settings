...
public class DeviceInfoSettings extends SettingsPreferenceFragment implements
    ...
    private static final String KEY_SWITCH_VERSION = "switch_version";
    ...
    private ListPreference mSwitchVersionPreference;
    private String mSwitchVersionStr;
    ...
    public void onCreate(Bundle icicle) {
        ...
        mSwitchVersionPreference = (ListPreference) findPreference(KEY_SWITCH_VERSION);
        if ("true".equals(SystemProperties.get("persist.switch.version.enabled", "false"))) {
            //String[] switch_version_entries = getResources().getStringArray(R.array.switch_version_entries);
            //String[] switch_version_values = getResources().getStringArray(R.array.switch_version_values);
            mSwitchVersionStr = SystemProperties.get("persist.switch.version.str", "default");
            mSwitchVersionPreference.setEnabled(true);
            mSwitchVersionPreference.setOnPreferenceChangeListener(this);
            //mSwitchVersionPreference.setSummary(switch_version_entries[0]);
            mSwitchVersionPreference.setValue(mSwitchVersionStr);
        }
    ...
    private void rebootToSwitchVersion(String verStr) throws IOException {
        Context context = mSwitchVersionPreference.getContext();
        Log.d(LOG_TAG, "SWITCH VERSION: " + verStr);
        FileOutputStream fout = new FileOutputStream("/data/misc/snbc_switch_version.tag");
        FileWriter fcmd = new FileWriter(fout.getFD());
        try {
            fcmd.write(verStr);
            //fcmd.write("\n");
            fcmd.close();
    ...
    private void cancelSwitchVersion() {
        mSwitchVersionPreference.setValue(mSwitchVersionStr);
    }

    private void checkToSwitchVersion(Context context, String verStr) {
        AlertDialog.Builder builder = new AlertDialog.Builder(context);
        builder.setTitle(context.getString(R.string.reboot_notice));
        builder.setMessage(context.getString(R.string.switch_version_notice));
        ...
        builder.setPositiveButton(context.getString(R.string.okay), new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                try {
                    rebootToSwitchVersion(verStr);
                } catch (IOException e) {
                    Log.e(LOG_TAG, "Reboot to switch version failed: " + e);
                }
            }
        });
    ...
    public boolean onPreferenceChange(Preference preference, Object objValue) {
        if (preference.getKey().equals(KEY_SWITCH_VERSION)) {
            String value = (String) objValue.toString();
            if (!value.equals(mSwitchVersionStr)) {
                checkToSwitchVersion(preference.getContext(), value);
            }
        }
        return true;
    }
