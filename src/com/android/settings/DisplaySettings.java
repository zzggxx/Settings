...
public class DisplaySettings extends SettingsPreferenceFragment implements
    ...
    private static final String KEY_SHOW_HIDE_STATUSBAR = "show_hide_statusbar";
	public static String ACTION_SHOW_STATUS_BAR = "com.snbc.action.show_status_bar";
    public static String ACTION_HIDE_STATUS_BAR = "com.snbc.action.hide_status_bar";
    private static final String KEY_DISPLAY_ROTATION_SET = "display_rotation_setting";
    ...
    private SwitchPreference mShowHideStatusbarPreference;
    private ListPreference mScreenOrientationPreference;
    private long mTime;

    private int mDisplayRotation;
    private int mDisplayRotationNew;
    private ListPreference mDisplayRotationPreference;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mDisplayRotationPreference = (ListPreference) findPreference(KEY_DISPLAY_ROTATION_SET);
        String strRot = SystemProperties.get("persist.sys.swrotation", "0");
        mDisplayRotation = rotationStrToIdx(strRot);
        mDisplayRotationNew = mDisplayRotation;
        String summary = mDisplayRotationPreference.getContext().getString(R.string.display_rotation_angle, strRot);
        mDisplayRotationPreference.setValue(strRot);
        mDisplayRotationPreference.setSummary(summary);
        mDisplayRotationPreference.setOnPreferenceChangeListener(this);
        ...
        if (isCameraGestureAvailable(getResources())) {
         ...
         mShowHideStatusbarPreference = (SwitchPreference) findPreference(KEY_SHOW_HIDE_STATUSBAR);
         mShowHideStatusbarPreference.setOnPreferenceChangeListener(this);
    ...
    private void updateState() {
        ...
        // Update show statusbar if it is available.
        if (mShowHideStatusbarPreference != null) {
            int hideBar = Settings.Global.getInt(getContentResolver(), Settings.Global.HIDE_STATUSBAR, 0);
            mShowHideStatusbarPreference.setChecked(hideBar == 1);
        }
    }
    ...
    private int rotationStrToIdx(String strRot) {
        int rot = 0;//Surface.ROTATION_0
        if(strRot.equals("90")){
            rot = 1;//Surface.ROTATION_90
        }else if(strRot.equals("180")){
            rot = 2;//Surface.ROTATION_180
        }else if(strRot.equals("270")){
            rot = 3;//Surface.ROTATION_270
        }
        return rot;
    }

    private String rotationIdxToStr(int rot) {
        String strRot = "0";
        if (rot == 1) {
            strRot = "90";
        } else if (rot == 2) {
    ...
    private void displayRotateAndReboot() {
        String strRot = rotationIdxToStr(mDisplayRotationNew);
        mDisplayRotation = mDisplayRotationNew;
        SystemProperties.set("persist.sys.swrotation", strRot);

        PowerManager pm = (PowerManager) mDisplayRotationPreference.getContext().getSystemService(Context.POWER_SERVICE);
        pm.reboot("");
    }

    private void cancelDisplayRotation() {
        String strRot = rotationIdxToStr(mDisplayRotation);
        String summary = mDisplayRotationPreference.getContext().getString(R.string.display_rotation_angle, strRot);
        mDisplayRotationPreference.setValue(strRot);
        mDisplayRotationPreference.setSummary(summary);
        mDisplayRotationNew = mDisplayRotation;
    ...
    private void showDisplayRotationDialog(Context context) {
        AlertDialog.Builder builder = new AlertDialog.Builder(context);
        builder.setTitle(context.getString(R.string.reboot_notice));
        builder.setMessage(context.getString(R.string.reboot_message));
    ...
    public boolean onPreferenceChange(Preference preference, Object objValue) {
        ...
        if(KEY_DISPLAY_ROTATION_SET.equals(key)){
            String strRot = (String)objValue.toString();
            mDisplayRotationNew = rotationStrToIdx(strRot);
            if (mDisplayRotationNew != mDisplayRotation) {
                showDisplayRotationDialog(preference.getContext());
            }
        }
        ...
        if (preference == mShowHideStatusbarPreference) {
            //fix double click bug
            if (System.currentTimeMillis() - mTime < 500) {
                return false;
            }
            mTime = System.currentTimeMillis();
            boolean hideBar  = (boolean) objValue; //mShowHideStatusbarPreference.isChecked();
            Settings.Global.putInt(getContentResolver(), Settings.Global.HIDE_STATUSBAR, hideBar ? 1 : 0);
